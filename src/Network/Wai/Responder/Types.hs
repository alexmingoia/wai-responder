module Network.Wai.Responder.Types where
...
import Data.Word
import Network.HTTP.Types (Header, Status, status400)
...
-- response. This provides convenient branching with do notation for redirects,
-- error responses, etc.
data ResponderM s a
  = ResponderM (ResponderState s -> IO (Either RouteAction (a, ResponderState s)))

data RouteAction = Respond Response

data ResponderOptions
  = ResponderOptions
      { parseBodyOpts :: ParseRequestBodyOptions }

data ResponderState s
  = ResponderState
      { appState :: s,
...
instance Functor (ResponderM s) where
  fmap f (ResponderM g) = ResponderM $ \r -> mapRight (\(a, b) -> (f a, b)) `fmap` g r

instance Applicative (ResponderM s) where
  pure a = ResponderM $ \r -> pure (Right (a, r))
  (<*>) = ap

instance Monad (ResponderM s) where
...
instance MonadIO (ResponderM s) where
  liftIO act = ResponderM $ \r -> act >>= \a -> return (Right (a, r))

instance MonadThrow (ResponderM s) where
  throwM = liftIO . throwIO

instance MonadCatch (ResponderM s) where
